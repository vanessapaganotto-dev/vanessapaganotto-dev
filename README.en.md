🇫🇷 Version française disponible ici → [README.md](README.md)

# Hi, I’m Vanessa 👋

I’m looking for an internship in web development / fronted - backend programming.  
Passionate about PHP, JavaScript, SQL, and database management.

---

## Technologies & Tools  
- PHP, JavaScript, HTML, CSS  
- MySQL, relational databases  
- Bootstrap, AJAX  
- Git, GitHub  
- Angular

---

## Key Projects  
- **Weekly Planning Management** — Interactive app to manage weekly reservations.  
- **Musical Notes Converter** — Dynamic conversion from classical to American notation.  
- **Educational Database** — Complete SQL dump with tables and sample data for course and assignment management.

---

## Contact  
📧 rachelpag19@gmail.com  
🔗 [LinkedIn](https://www.linkedin.com)  

Feel free to reach out to connect or collaborate! 🙂

---

## License

This project is licensed under the MIT License - see the [LICENSE](./LICENSE) file for details.

